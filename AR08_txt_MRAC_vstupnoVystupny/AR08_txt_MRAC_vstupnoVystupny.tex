...
\def\oznacenieCasti{AR08 - LS2019}
...
\begin{enumerate}
    ...
    \item Uvažujme riadený systém rovnako ako na cvičení siedmom, teda:
    \begin{align}
       G_{OP_1} &= 0,1659 \frac{s + 22}{ s^2 + 3,1423 s + 2,6539} 	\label{plantModel12}\\
       G_{OP_2} &= 0,1669 \frac{s + 20,7618}{s^2 + 2,3422s + 2,7293} \label{plantModel22}
    \end{align}
    pričom riadený systém pracuje v pásme danom dvomi pracovnými bodmi, ktorým zodpovedajú uvedené prenosové funkcie. Nominálna prenosová funkcia riadeného systému je daná tak, že jej koeficienty sú	priemery hodnôt oboch prenosových funkcií

    \begin{itemize}
    	\item Pre nominálnu prenosovú funkciu riadeného systému určte polynómy $Z_p$, $R_p$ a~zosilnenie $k_p$ pričom
    	\begin{equation} \label{C_PFsustavy_MRCp2}
    		\frac{y(s)}{u(s)} = k_p \frac{Z_p(s)}{R_p(s)}
    	\end{equation}
        kde $Z_p(s)$ je monický polynóm stupňa $m$, $R_p(s)$ je monický polynóm stupňa $n$ a $k_p$ je tzv. \emph{vysokofrekvenčné zosilnenie sústavy}. \emph{Relatívny stupeň} riadeného systému je $n^* = n - m$.
        \end{itemize}


	\item Pre nominálnu prenosovú funkciu riadeného systému navrhnite adaptívne riadenie s referenčným modelom so vstupno-výstupnou štruktúrou zákona riadenia (merateľný je len výstup riadenégho systému a samozrejme vstup). Pre odvodenie zákona adaptácie použite priamu Lyapunovou metódu. Nech referenčný model je v tvare
	\begin{equation}
...
\section{Cvičenie deviate}
\label{cvic9}

Pripravuje sa\footnote{No to určite\ldots}.




\section{Cvičenie desiate a jedenáste}
\label{cvic1011}


\subsection{Referát MRAC vstupno-výstupný pri ($n^* = 2$)}

Riadny termín odovzdania: do (vrátane) \textbf{\color{red} 01.05.2019} %.
...
\noindent
kde $H$ je počet bodov pridelených referátu, $H$ ako hodnotenie\footnote{Vonkoncom to nie je Hesseho matica! Čo nám pripomína, že ospravedlnenie patrí J. H. Bol to M. Ž. Mimochodom, prečo vôbec čítate poznámky pod čiarou?}. Päť a viac dní po riadnom termíne už nie je možné referát odovzdať a študent/študentka získa $0$~bodov. Minimálny počet bodov za referát je $0$~bodov. Maximálny počet bodov za referát je $15$ bodov.

\medskip

\noindent
Pre prácu na zadaní a referáte sú vyhradené cvičenia v~10. a~11. týždni semestra.
...
\begin{figure}[!t]
	...
	\makebox[\textwidth][c]{%
	\includegraphics{Obr_21_prevodovka.pdf}
	}

    \vspace{-2mm}

	\caption{Prevodová charakteristika kyvadla}
	\label{Obr prevodovka kyvadla}
...
\begin{figure}[!ht]
	...
	\makebox[\textwidth][c]{%
	\includegraphics{Obr_22_prevodovka.pdf}
	}

	\caption{Prevodová charakteristika a smernica prevodovej charakteristiky}
	\label{Obr prevodovka kyvadla a smernica}
...
\section{Cvičenie dvanáste}


Nech riadeným systémom je kyvadlo a nech riadenou veličinou je poloha (uhol) kyvadla. K tomu majme rovnicu opisujúcu dynamiku rotačného pohybu kyvadla. Rovnica je v tvare
\begin{equation}
    ml^2 \ddot{\varphi}(t) + \beta \dot{\varphi}(t) + mgl\sin{\varphi(t)} = u(t)
\end{equation}

Pre úplnosť: hmotný bod s hmotnosťou $m$ [kg] pripevnený na ramene so zanedbateľnou hmotnosťou a dĺžkou $l$ [m] kmitá, $o$ označuje os otáčania kolmú na rovinu, v ktorej kyvadlo kmitá. Kmity sú tlmené viskóznym trením s koeficientom $\beta$ [kg m$^2$ s$^{-1}$]. Uhol medzi zvislicou a ramenom kyvadla je označený $\varphi$ [rad] a gravitačné zrýchlenie $g$ [m s$^{-2}$]. Signál $u(t)$ [kg m$^2$ s$^{-2}$] je externý moment sily pôsobiaci na rameno kyvadla, $\dot{\varphi}(t)$ [rad s$^{-1}$] je uhlová rýchlosť a $\ddot{\varphi}(t)$ [rad s$^{-2}$] je uhlové zrýchlenie ramena kyvadla. Číselné hodnoty parametrov kyvadla sú nasledovné:
\begin{align*}
	m &= 1 \\
	l &= 1 \\
	g &= 9,81 \\
	\beta &= 2 \cdot 0,6 \cdot \sqrt{\frac{g}{l}}
\end{align*}
Viac o kyvadle nájsť možno v dokumente \emph{AR99\_oStabilite.pdf}.
...
Cieľom je riadiť polohu kyvadla v okolí rôznych pracovných bodov. Uvažujme však, že pracovné body sú z intervalu polôh kyvadla 0 až 90 stupňov. Konkrétna voľba pracovných bodov ako aj voľba ich okolia sa ponechávajú na čitateľa. Menovateľ aj tak nechodí na cvičenia.

Všimnime si však, že prevodová charakteristika kyvadla na danom intervale polôh kyvadla, je len pramálo aproximovateľná priamkou. Teda zosilnenie kyvadla sa mení vzhľadom na pracovný bod. To znamená, že jeden regulátor zrejme nedokáže zabezpečiť danú kvalitu riadenia v celom uvažovanom intervale výstupnej veličiny riadeného systému. Preto nech je použitý adaptívny regulátor, konkrétne MRAC v~zmysle tohto predmetu.

Kvalita riadenia, presnejšie, vlastnosti URO nech sú dané systémom druhého rádu s pólmi $p_1 = -\frac{1}{0.5}$ a $p_2 = -\frac{1}{0.4}$.



Tiež zobrazme, pre ilustráciu, prevodovú charakteristiku riadeného systému vzhľadom na uvažovanú úlohu, avšak len pre užší interval 30 až 70 stupňov - viď obr.~\ref{Obr prevodovka kyvadla}.


Nakreslime aj dotyčnice k prevodovej charakteristike napr. pre pracovné body dané vstupnými hodnotami $u_{PB1} = 5$ a $u_{PB1} = 9$. Viď obr.~\ref{Obr prevodovka kyvadla a smernica}. Je tak zrejmé, že zosilnenie systému je rôzne v týchto dvoch pracovných bodoch. Celkom presne určuje zmenu statického zosilnenia smernica prevodovej charakteristiky taktiež zobrazená na obr.~\ref{Obr prevodovka kyvadla a smernica}.

Mimochodom, je veľmi jednoduché nájsť analytické vyjadrenie prevodovej charakteristiky na základe modelu riadeného systému. V tomto prípade
\begin{equation}
    y_{PB} = \arcsin \left( \frac{1}{m g l} u_{PB} \right)
\end{equation}
kde $y_{PB}$ je výstupná veličina a $u_{PB}$ je vstupná veličina.

Týmto sme chceli len ilustrovať, že zrejme naozaj by bolo potrebné použiť iné parametre regulátora v pracovnom bode $u_{PB1} = 5$ ako v pracovnom bode $u_{PB1} = 9$.


\bigskip

\noindent
Prezentujte Vašu ochotu navrhnúť MRAC schopné riadiť systém aj pri zmenách pracovného bodu. Veľa šťastia.
