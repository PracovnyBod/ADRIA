...
figNamePrefix = 'fig_tex_03_'

figPlotParam = fcnDefaultFigSize(7.5, 0.12, 0.92, 0.1, 0.45, 13)
...
ax0.set_ylabel(u'[°]', y=1.0, ha='right', va='top', rotation='horizontal', )


ax0.plot(t_log, sig_r_ext * (180/np.pi),
         '-', lw=0.5, dashes=[8,5], color='gray',
         label=u'$r(t)$',
         )

ax0.plot(t_log, x_m_log[:,0] * (180/np.pi),
         '-k', lw=0.5, dashes=[6,3],
         label=u'$y_m(t)$',
         )


ax0.plot(t_log, y_log[:,0] * (180/np.pi),
         '-k', lw=1.0,
         label=u'$y(t)$',
         )
...
ax1.set_ylabel(u'[°]', y=1.02, ha='right', va='top', rotation='horizontal', )


ax1.plot(t_log, u_log * (180/np.pi),
         '-k', lw=1.0,
         label=u'$u(t)$',
         )
...
for ax in fig.get_axes():
    ...
    if ax == ax1:
        ax.legend(handles_ax, labels_ax, loc=1, bbox_to_anchor=(0.98, 1.2))
